...
def compute_rna_secondary_structures(
    ...
    secondary_structures_df[secondary_struct_fields] = secondary_structures_df.groupby(
        ...
        lambda group: get_secondary_struct(
            sequence_data_path=f"{sequence_data_dir}{re.sub('[^0-9a-zA-Z]+', '_', group.irow(0)['virus_species_name'])}_aligned.fasta",
            workdir=f"{workdir}/{re.sub('[^0-9a-zA-Z]+', '_', group.irow(0)['virus_species_name'])}/",

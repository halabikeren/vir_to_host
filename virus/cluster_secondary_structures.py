...
def is_positive(input_matrix: np.ndarray) -> bool:
    ...
    non_diagonal_values_positive = np.all(
        [input_matrix[i, j] > 0 for i in range(input_matrix.shape[0]) for j in range(input_matrix.shape[0]) if i != j]
    )
...
def compute_pairwise_distances(
    ref_structures: pd.Series, other_structures: pd.Series, workdir: str, output_dir: str, used_dist_type: str = "F"
) -> pd.DataFrame:
    """
    ...
    :param used_dist_type: distance type to use for the downstream clustering task
    :return:
    """
    ...
    i = 0
    batch_size = 1900
    ref_structures_batches = [ref_structures[i : i + batch_size] for i in range(0, len(ref_structures), batch_size)]
    for ref_structures_batch in ref_structures_batches:
        ...
        paths_exist = [
            len(os.listdir(f"{workdir}/rnadistance_out_{i}")) for i in range(starting_index, finishing_index)
        ]
        ...
        while not complete:
            ...
            logger.info(
                f"{len([item for item in paths_exist if item])} out of {len(paths_exist)} jobs are completed. will wait 2 minutes before checking again"
            )
            complete = np.all(paths_exist)
    ...
    distances_dfs = {
        dist_type: pd.DataFrame(index=ref_structures, columns=other_structures)
    ...
    return distances_dfs[
        used_dist_type
    ]  # use the distance computed based on the full (F) structure as it holds all 3 conditions of a distance metric (the HIT based distance also holds the triangle inequality


def get_distances_from_ref_structures(
    ref_structures: pd.Series, other_structures: pd.Series, workdir: str, distances_df: t.Optional[pd.DataFrame] = None
) -> float:
    """
    :param ref_structures: reference structures from which the distance to the other structures should be computed
    :param other_structures: structures to compute their distance from the reference structures
    ...
    if type(distances_df) != pd.core.frame.DataFrame:
        distances_df = compute_pairwise_distances(
            ref_structures=ref_structures,
            other_structures=other_structures,
            workdir=f"{workdir}/rnadistance_aux/",
...
def get_inter_cluster_distance(
    cluster_1_structures: pd.Series,
    cluster_2_structures: pd.Series,
    workdir: str,
    distances_df: t.Optional[pd.DataFrame] = None,
) -> float:
    """
    ...
    """
    return get_distances_from_ref_structures(
        ref_structures=cluster_1_structures,
        other_structures=cluster_2_structures,
        workdir=workdir,
...
def get_intra_cluster_distance(
    cluster_structures: pd.Series, workdir: str, distances_df: t.Optional[pd.DataFrame] = None
) -> float:
    """
    ...
    if len(cluster_structures) == 1:
        return 0
    return get_distances_from_ref_structures(
        ref_structures=cluster_structures,
        other_structures=cluster_structures,
        workdir=workdir,
        distances_df=distances_df,
    )


def compute_clusters_distances(
    clusters_data: pd.core.groupby.GroupBy, distances_df: pd.DataFrame, workdir: str, output_path: str
):
    """
    ...
    """
    df = pd.DataFrame(
        index=clusters_data.groups.keys(),
        columns=["intra_cluster_distance"] + [f"distance_from_{cluster}" for cluster in clusters_data.groups.keys()],
    )
    ...
    logger.info(
        f"computing intra-cluster distances for {len(clusters_data.groups.keys())} clusters of maximum size {max_cluster_size}"
    )
    intra_cluster_distances_workdir = f"{workdir}/intra_cluster_distances/"
    df["intra_cluster_distance"] = df.index.map(
        lambda cluster: get_intra_cluster_distance(
            cluster_structures=clusters_data.get_group(cluster).struct_representation,
            workdir=f"{intra_cluster_distances_workdir}/{cluster}",
            distances_df=distances_df,
        )
    )
    ...
    logger.info(
        f"computing inter-cluster distances across {int(len(clusters_data.groups.keys())**2/2)} combinations of clusters"
    )
    inter_cluster_distances_workdir = f"{workdir}/inter_cluster_distances/"
    for cluster2 in clusters_data.groups.keys():
        df[f"distance_from_{cluster2}"] = df.index.map(
            lambda cluster1: get_inter_cluster_distance(
                cluster_1_structures=clusters_data.get_group(cluster1).struct_representation,
                cluster_2_structures=clusters_data.get_group(cluster2).struct_representation,
                workdir=f"{inter_cluster_distances_workdir}/{cluster1}_{cluster2}_distances/",
                distances_df=distances_df,
            )
        )
    logger.info(
        f"mean inter-cluster distance = {np.mean(np.mean(df[[col for col in df.columns if 'distance_from_' in col]]))}"
    )
...
def get_optimal_clusters_num(
    kmin: int,
    kmax: int,
    clustering_df: pd.DataFrame,
    clustering_coordinates: t.List[np.array],
    cannot_link: t.List[t.Tuple[int]],
    distances_df: pd.DataFrame,
    workdir: str,
    use_upgma_based_starting_points: bool = True,
) -> t.Tuple[int, t.Dict[int, t.Dict], t.Dict[int, t.Dict]]:
    """
    ...
    """
    os.makedirs(workdir, exist_ok=True)
    coordinates_vectors = np.stack(clustering_coordinates)
        ...
        for k in range(kmin, kmax):  # switch with binary search , stop upn maximum of sl score
            if use_upgma_based_starting_points:
                if k in cluster_size_to_starting_points:
                    ...
                    if closest_k < k:
                        ...
                        additional_centers = [i for i in clustering_df.index if i not in starting_points_indices][
                            : (k - closest_k)
                        ]  # add k-closest_k points randomly
                        starting_points_indices += additional_centers
                    else:
                        starting_points_indices = random.sample(
                            population=cluster_size_to_starting_points[closest_k], k=k
                        )
            else:
            ...
            if clusters is not None:
                ...
                cluster_to_structures_indices = {
                    cluster: [
                        list(clustering_df.index)[i] for i in range(len(clustering_df.index)) if clusters[i] == cluster
                    ]
                    for cluster in unique_clusters
                }
                k_to_clusters_assignment[k] = dict()
                for i in clustering_df.index:
                    k_to_clusters_assignment[k][i] = [
                        cluster
                        for cluster in cluster_to_structures_indices
                        if i in cluster_to_structures_indices[cluster]
                    ][0]
    ...
    for k in range(kmin, kmax):
        ...
        ):  # under assumption of global maximum, if we got deterioration - we should stop at the prev value
            logger.info(
                f"silhouette score has been reduced with increase to {k} - possibly reached a local maxima at {k-1}"
            )
        prev_score = curr_score
    optimal_k = max(k_to_score, key=k_to_score.get)
    logger.info(
        f"the optimal number of clusters based on silhouette scores is {optimal_k} with a score of {k_to_score[optimal_k]}"
    )

    return optimal_k, k_to_clusters_assignment, k_to_cluster_centroids


def assign_cluster_by_homology(
    df: pd.DataFrame,
    sequence_data_dir: str,
    species_wise_msa_dir: str,
    workdir: str,
    distances_df: pd.DataFrame,
    coordinates: t.List[np.array],
    use_upgma_based_starting_points: bool = True,
) -> pd.DataFrame:
    """
    ...
    :param coordinates: coordinates of the structures in a euclidean space
    :param use_upgma_based_starting_points: indicator weather upgma based starting points should be used
    ...
    df.reset_index(
        inplace=True
    )  # must be done to provide cop_kmeans indices which correspond to the sub-coordinates and not all the coordinates
    ...
    # theory in: https://link.springer.com/content/pdf/10.1007/BF02288916.pdf
    best_clustering = {
        list(df.index)[i]: i for i in range(len(list(df.index)))
    }  # default clustering - cluster per structure
    best_clustering_centroids = {list(df.index)[i]: coordinates[i] for i in range(len(list(df.index)))}
    if df.shape[0] > 1:
        kmin = np.max(
            [2, np.max([data_by_sp.get_group(sp).shape[0] for sp in data_by_sp.groups.keys()])]
        )  # the number of cluster must equal at least the number of representatives per species, as two representatives of the same species cannot appear in the same cluster
        kmax = (
            df.shape[0] - 1
        )  # at the worst case, each structure will have its own cluster. we do not want to account for such case, which is naturally optimal, so we end and this number deducted by 1

        best_k, k_to_clusters_assignment, k_to_cluster_centroids = get_optimal_clusters_num(
            kmin=kmin,
            kmax=kmax,
            clustering_df=df,
            clustering_coordinates=coordinates,
            cannot_link=cannot_link,
            distances_df=distances_df,
            workdir=f"{workdir}/cop_clustering/",
            use_upgma_based_starting_points=use_upgma_based_starting_points,
        )
...
def plot_clusters(clusters_data: pd.DataFrame, structures_coordinates: t.List[np.array], output_path: str):
    """
    :param clusters_data: data with assigned cluster of each instance
    :param structures_coordinates: coordinates of the records in a euclidean space
    :param output_path: path to write the image to
    :return: none
    """
    logger.info(f"plotting the {len(clusters_data.assigned_cluster.unique())} clusters in {output_path}")
    clusters_data.assigned_cluster = clusters_data.assigned_cluster.astype(np.int)

    pca = PCA(n_components=2)
    pca.fit(np.stack(structures_coordinates))
    # translate each coordinate to its reduced representation based on the two PCs
    pc1 = pca.components_[0]
    pc2 = pca.components_[1]

    df = pd.DataFrame(columns=["x", "y", "cluster"])
    df["cluster"] = clusters_data["assigned_cluster"]

    for i in range(df.shape[0]):
        df.at[i, "x"] = np.dot(pc1, structures_coordinates[i])
        df.at[i, "y"] = np.dot(pc2, structures_coordinates[i])

    plt.figure(figsize=(40, 15))
    plot = sns.scatterplot(
        x="x", y="y", hue="cluster", palette=sns.color_palette("hls", len(df.cluster.unique())), data=df, legend="full"
    )
    h, l = plot.axes.get_legend_handles_labels()
    plot.axes.legend_.remove()

    plot.axes.legend(h, l, ncol=4)
...
@click.option(
    "--by", type=click.Choice(["homology", "column"]), help="path holding the output dataframe to write",
)
@click.option(
    "--sequence_data_dir",
...
@click.option(
    "--dist_type",
    type=click.Choice(["W", "P", "H", "F", "C", "edit_distance"]),
    help="distance type to use when computing the clustering solution",
    required=False,
    default="F",
)
def cluster_secondary_structures(
    structures_data_path: str,
    by: str,
    sequence_data_dir: str,
    species_wise_msa_dir: str,
    ...
    df_output_path = f"{workdir}/distances_all_structures/{dist_type}_distances.csv"
    ...
    # complement missing data by assuming that the distances df must be symmetric
    # fill missing values in the symmetric matrix
    ...
    if by != "column":
        ...
        structures_df = assign_cluster_by_homology(
            df=structures_df,
        ...
        logger.info(
            f"clustering data by {clustering_col} into {len(structures_df[clustering_col].dropna().unique())} groups"
        )
        structures_df[f"virus_hosts_{host_partition_to_use}_names"] = structures_df[clustering_col].apply(
    ...
    compute_clusters_distances(
        clusters_data=structures_df_by_clusters,
        distances_df=distances_df,
        workdir=f"{workdir}/structures_clusters_based_on_{dist_type}_distance_using_{'upgma' if use_upgma else 'random'}_sp/",
        output_path=f"{df_output_dir}/clusters_distances_based_on_{dist_type}_distance_using_{'upgma' if use_upgma else 'random'}_sp.csv",
    )

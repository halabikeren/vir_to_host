...
class TaxonomyCollectingUtils:
    @staticmethod
    def collect_taxonomy_data_from_gbif_api(
        df: pd.DataFrame, data_prefix: str
    ) -> pd.DataFrame:
        """
        ...
        record_names_with_missing_data = [
            ...
            for name in list(
                df.loc[
                    (df[f"{data_prefix}_taxon_id"].isna()), f"{data_prefix}_taxon_name"
                ].unique()
            )
        ]
        ...
        gbif_data = {
            name: species.name_suggest(q=name)
            for name in record_names_with_missing_data
        }
        gbif_relevant_data = {
            name: gbif_data[name][0] for name in gbif_data if len(gbif_data[name]) > 0
        }
        ...
        for key in gbif_lineage_keys_to_ncbi_keys:
            ...
            df[gbif_lineage_keys_to_ncbi_keys[key]].fillna(
                value=gbif_key_data, inplace=True
            )
        df.reset_index(inplace=True)

        # correct taxon names to scientific names
        avail_name_to_scientific_name = {
            name: gbif_relevant_data[name]["scientificName"].lower()
            for name in gbif_relevant_data
        }
        df[f"{data_prefix}_taxon_name"] = df[f"{data_prefix}_taxon_name"].replace(
            avail_name_to_scientific_name
        )
    ...
    @staticmethod
    def collect_taxonomy_data_from_ncbi_api(
        df: pd.DataFrame, data_prefix: str
    ) -> pd.DataFrame:
        """
        :param df: dataframe with taxonomy data to fill
        ...
        species_names = list(
            df.loc[
                df[f"{data_prefix}_species_id"].isna(), f"{data_prefix}_species_name"
            ].unique()
        )
        ...
        while i < len(species_names):
            ...
            try:
                species_name_to_data[name] = Entrez.read(
                    Entrez.esearch(db="taxonomy", term=name, retmode="xml")
                )
                i += 1
        ...
        species_name_to_id = {
            ...
            if "IdList" in species_name_to_data[name]
            and len(species_name_to_data[name]["IdList"]) > 0
        }
    ...
    @staticmethod
    def collect_tax_ids(
        df: pd.DataFrame, taxonomy_names_df: pd.DataFrame, data_prefix: str
    ) -> pd.DataFrame:
        """
        :param df: dataframe with taxonomy data to fill
    ...
    @staticmethod
    def collect_lineage_info(
        df: pd.DataFrame, taxonomy_lineage_df: pd.DataFrame, data_prefix: str
    ) -> pd.DataFrame:
        """
        :param df: dataframe with taxonomy data to fill
        ...
        for field in fill_by_fields.keys():
            if (
                field in df.columns
                and fill_by_fields[field] in taxonomy_lineage_df.columns
            ):
                ...
                relevant_df.to_csv(
                    f"{os.getcwd()}taxonomy_lineage_df_missing_values.csv", index=False
                )
    ...
    @staticmethod
    def collect_tax_rank(
        df: pd.DataFrame, taxonomy_ranks_df: pd.DataFrame, data_prefix: str
    ) -> pd.DataFrame:
        """
        :param df: dataframe with taxonomy data to fill
        ...
        if f"{data_prefix}_species_id" not in df.columns:
            df[f"{data_prefix}_species_id"] = np.nan
        df.loc[
            (df[f"{data_prefix}_species_id"].isna())
            & (df[f"{data_prefix}_taxon_rank"] == "species"),
    ...
    def collect_taxonomy_data(
        ...
        logger.info(
            f"# missing data before taxonomy data collection from ftp = {df.isnull().sum()}"
        )
        ...
        df = TaxonomyCollectingUtils.collect_tax_ids(
            df=df, taxonomy_names_df=taxonomy_names_df, data_prefix="virus"
        )
        df = TaxonomyCollectingUtils.collect_tax_ids(
            df=df, taxonomy_names_df=taxonomy_names_df, data_prefix="host"
        )
        ...
        #                                         num_of_processes=2)  # multiprocessing.cpu_count())
        df = TaxonomyCollectingUtils.collect_taxonomy_data_from_ncbi_api(
            df=df, data_prefix="virus"
        )
        # df = ParallelizationService.parallelize(df=df, func=partial(
        ...
        df = TaxonomyCollectingUtils.collect_tax_rank(
            df=df, taxonomy_ranks_df=taxonomy_ranks_df, data_prefix="virus"
        )
        df = TaxonomyCollectingUtils.collect_tax_rank(
            df=df, taxonomy_ranks_df=taxonomy_ranks_df, data_prefix="host"
        )

        df.to_csv(output_path, index=False)

        logger.info(
            f"# missing data after taxonomy data collection from ftp = {df.isnull().sum()}"
        )
